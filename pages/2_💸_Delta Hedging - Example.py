...
if df_price["St"].iloc[-1] > K_A: 
    if CP_A=="Short Call":  moneyness = "ITM"
    if CP_A=="Short Put":  moneyness = "OTM"
if df_price["St"].iloc[-1] < K_A: 
    if CP_A=="Short Call":  moneyness = "OTM"
    if CP_A=="Short Put":  moneyness = "ITM"

#%% === B區: 股價 & 權證價圖 ===
st.info(f"""目前參數:　　:red[S0]={S0},　　:red[K]={K_A},　　:red[r]={r},　　:red[T]={round(T,2)},　　:red[sigma]={sigma} 
        \n 　　　　　　:red[type]={CP_A},　　:red[sell price]={sell_price},　　:red[情境]={St_sce},　　:red[期末]={moneyness}""")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📈 Delta避險不同頻率比較", "📚 Delta避險各部位損益","📚 每期避險","📚 每2期避險", "📚 每5期避險", "📚 靜態避險"])
# D-tab1
...
tab3.markdown(f"""避險成本={round(cost,2)}""")
tab3.dataframe(df_delta)
tab4.markdown(f"""避險成本={round(cost2,2)}""")
tab4.dataframe(df_delta2)
tab5.markdown(f"""避險成本={round(cost5,2)}""")
tab5.dataframe(df_delta5)
tab6.markdown(f"""避險成本={round(cost20,2)}""")
tab6.dataframe(df_delta20)

# D-2 Delta避險各部位損益
delta_sce = tab2.selectbox( """選擇不同頻率避險方式""", cname[0][1:] ) #,label_visibility ="collapsed"
delta_df_list = {"Delta1": df_delta, "Delta2": df_delta2, "Delta5": df_delta5, "Delta20":df_delta20}
fig = px.line(delta_df_list[delta_sce].round(2), x="t", y=["A部位損益","現貨部位損益","總損益"], title=delta_sce+" 避險各部位損益", \
...
fig1 = px.line(df_all_hedge.round(2), x="t", y=cname, title="Delta、Delta-Gamma避險比較", \
               labels={"value":"profit"},height=400, width=700, template="plotly_white")
...
df_all_hedge = pd.DataFrame()
df_all_hedge["t"] = df_delta["t"]
df_all_hedge = pd.concat([df_all_hedge,df_delta["A部位損益"],df_delta["總損益"],df_gamma2["總損益"]], axis=1)
df_all_hedge.columns = ["t"]+cname

fig1 = px.line(df_all_hedge.round(2), x="t", y=cname, title="Delta-Gamma避險損益", \
               labels={"value":"profit"},height=400, width=700, template="plotly_white")
fig2 = px.line(df_gamma2.round(2), x="t", y=["A部位損益","B部位損益","現貨部位損益","總損益"], title="Delta-Gamma避險", \
               labels={"value":"profit"},height=400, width=700, template="plotly_white") 
#tab1, tab2, tab3 = st.tabs(["📈 Delta-Gamma避險損益", "📈 各部位損益","📚 Data"])
#tab1.plotly_chart(fig1)
#tab2.plotly_chart(fig2)
#tab3.dataframe(df_gamma2)
